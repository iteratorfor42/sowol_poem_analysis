"""
analyze.py 
[기존 이력에 '내일 리팩토링할 것'이란 표현이 있는데, (자세한 건GIT COMMIT LOG에서 확인 가능)
실제 재확인은 26.09.22에 했고 별도의 리팩토링은 하지 않았다. 
그저 '이 저장소'와 '이 저장소의 산출물 = 실제 dh_webpage의 결과 html'을 재확인했다.]
----------
KPoEM 모델로 시 전체/행 단위 감정 분석을 수행하고, 
인문학적으로 활용 가능한 구조화된 레코드로 변환한다.

OCR 프로젝트(postprocess.py의 to_structured_record)와 동일한 문제의식을 공유한다.
"분석 결과를 단순 수치 나열이 아니라, 시-행-감정이 연결된 구조화된 데이터로 남겨야 
이후 시각화·비교·군집화 등으로 이어질 수 있다."
"""
...
def analyze_poem(
    ...
    """
    시 한 편을 분석. 시 전체 텍스트로 한 번, 
    (옵션) 행 단위로 각각 한 번씩 KPoEM 모델에 통과시킨다.

    주의: 시행 하나하나는 문맥이 짧아 모델이 학습한 입력 길이(문장~짧은 문단)와
    비교적 잘 맞는 편이지만, 시 전체를 한 번에 넣으면 512 토큰 제한 안에서
    여러 정서가 뭉뚱그려질 수 있다. 그러니 두 레벨을 같이 보는 것을 권장.
    """
