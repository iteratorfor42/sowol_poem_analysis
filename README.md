...
>김소월 시집 『진달래꽃』 중 "바다가 변하야 뽕나무밭 된다고" 섹션에 실린 9편을, 
>한국학중앙연구원 디지털인문학연구소(AKS-DHLAB)가 공개한 KPoEM
...
- 시 원문은 `https://ko.wikisource.org/wiki/진달래꽃_(시집)`에서 가져왔습니다.
...
KPoEM 모델 자체가 이 두 라이브러리 위에서 동작하기 때문에 **필수**입니다.) 
...
시간이 걸릴 수 있습니다 (이후에는 로컬 캐시 사용. 최소 RAM 8 이상은 되어야 돌아갈 듯합니다.).
